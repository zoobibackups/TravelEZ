...
const SectionListDropDown = ({
  ...
  const getSuggestions = query => {
    ...
    if (first_location == null) {
      url = `https://v2.holidayextras.co.uk/search/api/resort-transfers-destination?query=${filterToken}`;
    } else {
      url = `https://v2.holidayextras.co.uk/search/api/resort-transfers-destination?query=${filterToken}&code=${first_location.lat},${first_location.lng}`;
    }
    fetch(url, requestOptions)
      .then(response => response.json())
      .then(response => {
        const sections = [];
        const my_data = groupBy(response, 'locationType');
        Object.keys(my_data).map((item, i) => {
          sections.push({
            title: item,
            data: my_data[item],
          });
    ...
    <Modal
      ...
      <View style={styles.main_view}>
        ...
        <SectionList
          ...
          ListHeaderComponent={
            <TouchableOpacity style={styles.sectionheaderStyle}>
              <FontAwesome
                name={'circle-o'}
                style={{marginRight: wp(2)}}
                color={'#a1aac4'}
          ...
          renderSectionHeader={({section: {title}}) => (
            <View style={styles.seactionheadertitleView}>
              {title != 'AP' ? (
                <FontAwesome
                  name={'hotel'}
                  style={{marginRight: wp(2)}}
